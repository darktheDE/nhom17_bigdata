...
# -*- coding: utf-8 -*-
"""
Local test runner for MapReduce jobs
Simulates Hadoop Streaming pipeline without shell dependencies
"""
...
def run_mapreduce_local(input_files, output_file, mapper_script, reducer_script):
    """
    Run MapReduce job locally using Python subprocess
    """
    print(f"Starting MapReduce job...")
    print(f"Input files: {input_files}")
    print(f"Output: {output_file}")
    
    # Step 1: Run mapper on all input files
    mapper_output = []
...
if __name__ == '__main__':
    ...
    output_file = os.path.join(base_dir, 'data', 'output_mapreduce', 'job01_avg_price_by_brand.txt')
    mapper_script = 'mapper.py'
    reducer_script = 'reducer.py'
    
    print("=" * 70)
    print("Job 01: Average Price by Brand")
    print("=" * 70)
    
    success = run_mapreduce_local(input_files, output_file, mapper_script, reducer_script)
